...
function _inv_space(rep, chi , basis, smooth::Bool=true) 
  _rep2 = [rep[i]-chi[i][1]*identity_matrix(base_ring(rep[i]), ncols(rep[i])) for i=1:length(rep)]
  rep2 = reduce(vcat, _rep2)
  dim, _V = kernel(rep2)
  if dim == 0
    return false
  end
  V = [transpose(_V[:,i]) for i=1:ncols(_V)]
  if dim == 1
...
function _ideal_degeneracy(X, Mfact, k)
  ...
  for l=1:length(val)
    # val[l] = reduce(val[l], groebner_basis(saturated_ideal(OO(X).J)))
